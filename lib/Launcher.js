...
class Launcher {
   ...
   */
  static async launch(options = {}, detached=undefined) {
    const {
    ...
    const chromeProcess = childProcess.spawn(
        ...
        chromeArguments,
        {
          ...
          detached: typeof detached === 'undefined' ? process.platform !== 'win32' : detached,
          env,
          stdio
        }
    );
    ...
    const listeners = [ helper.addEventListener(process, 'exit', detached ? null : killChrome) ];
    if (handleSIGINT)
